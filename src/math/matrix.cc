...
{
  for ( int lj=0; lj < nblocks(); lj++ )   // nblocks: number of local blocks
  {
    for ( int jj=0; jj < nbs(lj); jj++ )   // nbs: size of blocks , similar to nb_ but take into account last block
    {
      const int nglobal = j(lj,jj);   // global state index,  (lj * npcol_ + mycol_) * nb_ + jj                               
      const int norig = lj*nb()+jj;   // local index
      for ( int li = 0; li < mblocks(); li++ )
      {
        for ( int ii=0; ii < mbs(li); ii++)
        {
          const int mglobal = i(li,ii); // global state index,  (li * nprow_ + myrow_) * mb_ + ii --> obviously out of bound if mb=nb  
          const int morig = li*mb()+ii;
          occ_result[nglobal] += norm(val[morig+mloc_*norig])*occ_current[mglobal];
          //cout << "mglobal,nglobal" << " at " << myrow_ << " , " << mycol_ << " = " << mglobal << " , " << nglobal << endl;
          //cout << "occ_current at " << mglobal << " = " <<  occ_current[mglobal] << endl;
        }
      }
    }
  }
  ...
  {
    int size = n_;
    //cout << "size of occ_result " << size << endl; 
    MPI_Allreduce(MPI_IN_PLACE, &occ_result[0], size, MPI_DOUBLE, MPI_SUM, ctxt_.comm() );
  }
}
