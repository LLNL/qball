...
int main(int argc, char **argv)
{
   ...
   {  
      ...
      if (tortho) {
         ...
         //cout << "M = " << m_c << " N = " << n_c << " K = " << m_a << " LDA = " << m_a << " LDB = " << m_a << " LDC = " << m_c << " dgemm time = " << setprecision(5) << setw(8) << tm.real()<< " sec, GFlops = " << (2.0e-9*m_c*n_c*m_a) / tm.real() << endl;
...
/*
         ...
         if ( tm.real() > 0.0 )
         {
            int kmax = ( ta == 'n' ) ? a.n() : a.m();
            cout << "  GFlops: " 
                 << (2.0e-9*m_c*n_c*kmax) / tm.real() << endl;
         }
*/
#if 0    
         double norma=a.nrm2();
