...
//
////////////////////////////////////////////////////////////////////////////////
// $Id: EnergyFunctional.C,v 1.51 2010/08/30 20:55:43 draeger1 Exp $
  ...
  */
  ...
  if (s_.ctrl.ultrasoft) {
    for ( int ispin = 0; ispin < wf_.nspin(); ispin++ ) {
      ...
      vft->forward(&vrtmp[0],&veff_g[ispin][0]);    
    }
  }
...
double EnergyFunctional::energy(bool compute_hpsi, Wavefunction& dwf,
  ...
  */
  ...
  if ( compute_forces ) {
    ...
    if (s_.ctrl.ultrasoft && 1 == 0) {
      
      // force from vps*drhogus/dR_ion
      vector<vector<complex<double> > > drhog;

      //int ml = vbasis_->maxlocalsize();
      //drhog.resize(3);
      //for (int j=0; j<3; j++)
      //  drhog[j].resize(ml);

      cd_.calc_drhogus(drhog);
      ...
      for ( int is = 0; is < nsp_; is++ ) {
        ...
        if (s->ultrasoft()) { 
          ...
          for ( int ia = 0; ia < na_[is]; ia++ ) {
            double sum0=0.0,sum1=0.0,sum2=0.0;
            for ( int ig = 0; ig < ngloc; ig++ ) {
              const double arg = tau0[is][3*ia]*gx0[ig] + tau0[is][3*ia+1]*gx1[ig] + tau0[is][3*ia+2]*gx2[ig];
              complex<double> sfvg = vps[is][ig]*complex<double>(cos(arg),-sin(arg));
              sum0 += real(sfvg*conj(drhog[0][ig]));
              sum1 += real(sfvg*conj(drhog[1][ig]));
              sum2 += real(sfvg*conj(drhog[2][ig]));
            }            
            ftmp[3*ia+0] = sum0;
            ftmp[3*ia+1] = sum1;
            ftmp[3*ia+2] = sum2;
          ...
          //ewd DEBUG
          //if (s_.ctxt_.oncoutpe()) { 
          //  for ( int i = 0; i < na_[is]; i++ ) {
          //    cout << "EF.FORCES, ia = " << i << ", fion_usel1 = " << -vfac*ftmp[3*i+0] << " " << -vfac*ftmp[3*i+1] << " " << -vfac*ftmp[3*i+2] << endl;
          //  }
          //}
          //ewd DEBUG
          
          for ( int i = 0; i < 3*na_[is]; i++ )
            fion[is][i] -= vfac * ftmp[i];
        }      
      }

      // force from d(ehart)/dR_I = -8*pi*omega SUM_ig conj(rhogt[ig])*drhogus*g2i
      for ( int is = 0; is < nsp_; is++ ) {
        Species *s = s_.atoms.species_list[is];
        if (s->ultrasoft()) { 
          memset((void*)&ftmp[0],0,3*namax_*sizeof(double));
          for ( int ia = 0; ia < na_[is]; ia++ ) {
            double sum0=0.0,sum1=0.0,sum2=0.0;
            for ( int ig = 0; ig < ngloc; ig++ ) {
              sum0 += g2i[ig]*real(conj(rhogt[ig])*drhog[0][ig]);
              sum1 += g2i[ig]*real(conj(rhogt[ig])*drhog[1][ig]);
              sum2 += g2i[ig]*real(conj(rhogt[ig])*drhog[2][ig]);
            }
            ftmp[3*ia+0] = fpi*sum0;
            ftmp[3*ia+1] = fpi*sum1;
            ftmp[3*ia+2] = fpi*sum2;
          }
          int len = 3*na_[is];
          vbasis_->context().dsum(len,1,&ftmp[0],len);
          double vfac = vbasis_->real() ? 2.*omega : omega;
          
          //ewd DEBUG
          //if (s_.ctxt_.oncoutpe()) { 
          //  for ( int i = 0; i < na_[is]; i++ ) {
          //    cout << "EF.FORCES, ia = " << i << ", fion_usel2 = " << -vfac*ftmp[3*i+0] << " " << -vfac*ftmp[3*i+1] << " " << -vfac*ftmp[3*i+2] << endl;
          //  }
          //}
          //ewd DEBUG
